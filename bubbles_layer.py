...
class BubblesLayer(QObject):
    ...
    def __init__(self, widget, *, count=18, min_r=14, max_r=55,
        ...
        self._string_max_d = min(self._w, self._h) * 0.28  # connection range ≈ 28% of screen
    ...
    def paint(self, p):
        ...
        Also draws faint connecting strings between nearby bubbles."""
        if not self._bubbles:
            return
        p.setRenderHint(QPainter.Antialiasing)

        # ── Connecting strings first (so they sit behind bubbles) ────────
        bubs = self._bubbles
        max_dist = self._string_max_d  # px; bubbles closer than this get strung
        for i, bi in enumerate(bubs):
            for bj in bubs[i + 1:]:
                d = math.hypot(bi.cx - bj.cx, bi.cy - bj.cy)
                if d < max_dist:
                    alpha = int(self.max_alpha * 0.35 * (1.0 - d / max_dist))
                    p.setPen(QColor.fromHsl(
                        (bi.hue + bj.hue) // 2, 40, 60, alpha))
    ...
    def on_resize(self, w, h):
        ...
        self._string_max_d = min(w, h) * 0.28
